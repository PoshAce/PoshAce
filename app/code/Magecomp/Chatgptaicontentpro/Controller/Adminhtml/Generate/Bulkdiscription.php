...
class Bulkdiscription extends Action
{
    ...
    public function execute()
    {
        ...
        foreach ($selectedProducts as $product) {
             ...
             $type = $this->completionConfig->getByType('description');
             $prompt=$productNames;
             $result = $type->query($prompt);  
             $product1->setStoreId(0);
             $product1->setDescription($result); 
             $this->productRepository->save($product1);
                              
        } 
         $this->messageManager->addSuccessMessage(__('Description Updated Successfully for '.$process.' Products'));

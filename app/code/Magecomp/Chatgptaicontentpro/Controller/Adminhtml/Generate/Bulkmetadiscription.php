...
class Bulkmetadiscription extends Action
{
    ...
    public function execute()
    {
        ...
        foreach ($selectedProducts as $product) {
             ...
             $type = $this->completionConfig->getByType('meta_description');
        
             $prompt=$productNames;
             $result = $type->query($prompt);  
             $product1->setStoreId(0);
             $product1->setMetaDescription($result); 
             $this->productRepository->save($product1);
                              
        } 
         $this->messageManager->addSuccessMessage(__('Meta Description Updated Successfully for '.$process.' Products'));

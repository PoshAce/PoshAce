...
class Bulkmetatitle extends Action
{
    ...
    public function execute()
    {
        ...
        foreach ($selectedProducts as $product) {
             ...
             $type = $this->completionConfig->getByType('meta_title');
             $prompt=$productNames;
             $result = $type->query($prompt);  
             $product1->setStoreId(0);
             $product1->setMetaTitle($result); 
             $this->productRepository->save($product1);
                              
        } 
         $this->messageManager->addSuccessMessage(__('Meta Title Updated Successfully for '.$process.' Products'));

...
class Bulkmetakeywords extends Action
{
    ...
    public function execute()
    {
        ...
        foreach ($selectedProducts as $product) {
             ...
             $type = $this->completionConfig->getByType('meta_keywords');
        
             $prompt=$productNames;
             $result = $type->query($prompt); 
             $product1->setStoreId(0);
             $product1->setMetaKeyword($result); 
             $this->productRepository->save($product1);
                              
        } 
         $this->messageManager->addSuccessMessage(__('Meta Keywords Updated Successfully for '.$process.' Products'));

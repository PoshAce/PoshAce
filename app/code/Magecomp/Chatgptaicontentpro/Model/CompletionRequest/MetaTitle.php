    ...
    public function getApiPayload(string $text): array
    {
       ...
       $storeid = $this->request->getParam('storeid');
        ...
        if (str_contains($text, "categorypage")) {
            ...
            $configprompt = "Create meta title (only content) of the following category without double inverted commas:, $test .'generate in '.$languages.' language'";
        } else if (str_contains($text, "cmspage")) {
            $test = str_replace(',cmspage', '', $text);
            $configprompt = "Create meta title (only content) of the following cms page without double inverted commas:, $test .'generate in '.$languages.' language'";
        } else {
            $configprompt = "Create meta title (only content) of the following product without double inverted commas:, $text .'generate in '.$languages.' language'";
        }

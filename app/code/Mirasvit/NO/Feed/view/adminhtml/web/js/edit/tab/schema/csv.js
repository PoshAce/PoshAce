define([
    'jquery',
    'ko',
    'uiComponent',
    'underscore',
    'collapsable',
    'schema'
], function ($, ko, Component, _) {
    'use strict';

    return Component.extend({
        defaults: {
            template: 'Mirasvit_Feed/edit/tab/schema/csv'
        },

        initialize: function () {
            var self = this;
            $('body').trigger('processStart');
            this._super();

            _.bindAll(this, 'removeRow');

            self.tmp = ko.observableArray([]);

            _.each(self.rows, function (row) {
                var obj = new $.Pattern().load(row);
                self.tmp.push(obj);
            });

            self.rows = self.tmp;
        },

        afterRender: function (element) {
            $('[data-role=row]', element).collapsable();

            $('[data-role=sortable], [data-role=sortable]', element).sortable({
                distance: 8,
                tolerance: 'pointer',
                axis: 'y',
                update: function () {
                    $('[data-role=order]', this).each(function (index, element) {
                        $(element).val(index + 1);
                    });
                }
            });
            $('body').trigger('processStop');
        },

        addRow: function () {
            var pattern = new $.Pattern().opened(true);
            this.rows.push(pattern);
        },

        removeRow: function (row, event) {
            let rowElement = $(event.target).closest('.schema-pattern');
            $(rowElement).css({'display': 'none'});
            $(rowElement).find('input.delete-flag').val(1);
        },

        toggle: function (row, event) {
            row.opened(!row.opened());
        }
    });
});

define([
    ...
    $.Pattern = function () {
        ...
        this.removedModifiers = ko.observableArray([]);
        ...
        this.build = function () {
            ...
            _.each(self.modifiers(), function (modifier) {
                ...
                _.each(self.removedModifiers(), function (removedModifier) {
                    if (removedModifier === modifier) {
                        isRemoved = true;
                    }
                });

                if (!isRemoved && modifier.toString()) {
        ...
        this.removeModifier = function (modifier, event) {
            let modifierElement = $(event.target).closest('.schema-modifier');
            $(modifierElement).css({'display': 'none'});
            $(modifierElement).find('input.delete-flag').val(1);
            self.removedModifiers().push(modifier);
            self.build();
        };
        ...
        this.removeValidator = function (validator, event) {
            let validatorElement = $(event.target).closest('.schema-modifier');
            $(validatorElement).css({'display': 'none'});
            $(validatorElement).find('input.delete-flag').val(1);
        };

        this.setArgumentsCount = function (modifier, event) {
            let modifierElement = $(event.target).closest('.schema-modifier'),
                argCount = modifier.args().length;
            $(modifierElement).find('input.arg-count').val(argCount);
        }
    };

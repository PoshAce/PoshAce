...
use Mirasvit\Feed\Export\Resolver\Product\ImageResolver;
...
class Product extends AbstractCondition
{
    ...
    public function __construct(
        ...
        Yesno                      $systemConfigSourceYesno,
        ItemFactory                $stockItemFactory,
        AttributeCollectionFactory $entityAttributeSetCollectionFactory,
    ...
    public function getSqlCondition(Collection $collection): string
    {
    ...
    public function getValueOption($option = null): string
    {
    ...
    public function collectValidatedAttributes(Collection $productCollection): self
    {
        ...
        if (!in_array($attribute, ['entity_id', 'category_ids', 'qty', 'qty_inventory', 'final_price', 'children_count', 'php', 'is_in_stock', 'is_in_stock_inventory', 'manage_stock', 'manage_stock_inventory', 'status_parent', 'is_salable'])) {
    ...
    public function getInputType(): string
    {
        if ($this->getAttribute() === 'attribute_set_id' ||
            ...
            $this->getAttribute() === 'is_in_stock_inventory' ||
    ...
    public function getChildrenInStock(AbstractModel $object): Collection
    {
     ...
     * @return bool|int
     */
    public function getMsiStock(ModelProduct $object, string $attrCode)
    {
        if (
    ...
    public function validate(AbstractModel $object): bool
    {
        ...
        switch ($attrCode) {
            ...
            case 'is_salable':
                ...
                if ($object->getVisibility() == Visibility::VISIBILITY_NOT_VISIBLE) {
            ...
            case 'thumbnail_size':
                ...
                $path      = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath() . '/catalog/product' . $imagePath;
    ...
    protected function _prepareValueOptions(): self
    {
        ...
        if ($this->getAttribute() === 'attribute_set_id') {
            ...
            $selectOptions = $this->attributeSetCollectionFactory->create()
                ...
                ->load()
                ->toOptionArray();
        } elseif ($this->getAttribute() === 'is_in_stock' || $this->getAttribute() === 'is_in_stock_inventory') {
            $selectOptions = [];
    ...
    private function addSpecialAttributes(array &$attributes)
    {
        $attributes = array_merge($attributes, [
            ...
            'is_in_stock_inventory'  => __('Stock Availability (Multi Source Inventory)'),

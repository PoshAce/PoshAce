...
class TemplateVarManager extends \Magento\Framework\DataObject
{
    ...
    public function collect()
    {
        ...
        $this->collectMtVariables();
    }

    public function collectMtVariables()
    {
        if (isset($this->vars['shipment']) && $this->vars['shipment']) {
            $this->addTrackingLink($this->vars['shipment']);
            $this->addPopupTrackingLink($this->vars['shipment']);
        }
    }

    public function addTrackingLink($shipment)
    {
        foreach ($shipment->getAllTracks() as $item) {
            $this->setData('tracking_link', $this->getTrackinkLinkByItem($item));
            break;
        }
    }

    public function addPopupTrackingLink($shipment)
    {
        $this->setData('popup_tracking_link', $this->shippingHelper->getTrackingPopupUrlBySalesModel($shipment));
    }

    public function getDesignCss($template)
    {
        $origTemplateCode = $template->getOrigTemplateCode();
        if (!$origTemplateCode || strpos($origTemplateCode, 'mt_email_') === false) {
            return '';
        }

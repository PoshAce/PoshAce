...
class ImportManager
{
     ...
     * @param ResourceModel\Variable\CollectionFactory $varCollectionFactory
    ...
    public function updateVariableCollection($template, $templateData)
    {
        ...
        $storeUrls = [];
        if (isset($variableList['store_urls'])) {
            $storeUrls = $variableList['store_urls'];
            unset($variableList['store_urls']);
        }

        foreach ($variableList as $varData) {
            $varData = $this->updateUrls($varData, $storeUrls);
    ...
    public function updateUrls($varData, $storeUrls = [])
    {
        if (!empty($storeUrls) && isset($varData['var_value'])) {
            $storeId = $varData['store_id'];
            ...
            foreach ($storeUrls as $url) {
                $varData['var_value'] = str_replace($url, $storeUrl, $varData['var_value']);
            }
        }

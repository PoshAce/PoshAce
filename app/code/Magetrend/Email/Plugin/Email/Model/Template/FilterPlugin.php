...
class FilterPlugin
{
    ...
    /**
     * @var \Magetrend\Email\Model\TemplateVarManager
     */
     ...
     */
    public function beforeSetVariables(Filter $filter, array $variables)
    {
        if (!empty($variables) && isset($variables['template_styles'])) {
            $this->templateStyles = $variables['template_styles'];
        }

        if (!empty($variables) && isset($variables['this'])) {
            $this->template = $variables['this'];
        }

        $this->_registry->unregister('mtemail_template_filter_allow_process');
        if ($this->template && $this->template->getData('is_mt_email') == 1) {
            $this->_registry->register('mtemail_template_filter_allow_process', 1);
        }
    ...
    public function afterFilter(Filter $filter, $value)
    {
        ...
        if (substr_count($origValue, '{{layout handle="mtemail_') > 0) {
        ...
        try {
            ...
            if ($this->_appState->getMode() == \Magento\Framework\App\State::MODE_DEVELOPER) {
                $value = __('Error filtering template: %1', $e->getMessage());
            } else {
    ...
    protected function isAllowToProcess()
    {
        ...
        if ($this->_registry->registry('mtemail_template_filter_allow_process') != 1) {
            return false;
        }
    ...
    protected function addBody($value)
    {
        if (substr_count($value, '<body>') == 0) {
            $value = $this->_blockFactory->createBlock('Magento\Framework\View\Element\Template')
                ...
                ->setTemplateCss($this->templateStyles)
                ->setDesignHeadHtml($this->templateVarManager->getDesignHeadHtml($this->template))
                ->setDesignCss($this->templateVarManager->getDesignCss($this->template))
                ->setArea('frontend')
                ->toHtml();
        }

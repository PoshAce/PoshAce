...
class Data
{
    ...
    public function getDemoVars($template)
    {
        ...
        $vars = [
            ...
            'subscriber' => $this->getDemoSubscriber($store),
            'store' => $store,
            'order' => $order,
            ...
            'coupon_code' => 'XXXX-0000',
            'coupon_expire_date' => date('Y-m-d'),
        ];
        ...
        $vars = $this->addDummyProductData($vars, $store);
    ...
    public function addDummyProductData($vars, $store)
    {
        $helper = $this->imageHelper->create();
        $placeHolder = $this->assetRepos->getUrl($helper->getPlaceholder('image'));
        $vars['product_name'] = __('Dummy Name');
        $vars['product_url'] = 'http://store.demo.store.com/product.url.html';
        $vars['product_image'] = $placeHolder;
        $vars['product_options'] = __('Color: red');
        $vars['product_price'] = '$19.99';

        $sku = $this->scopeConfig->getValue(
            'mtemail/product_block/sku_1',
        ...
        if ($product) {
            ...
            $vars['product_url'] = $product->getProductUrl();
            $vars['product_image'] = $this->getProductImage($product);
            $vars['product_options'] = __('Color: red');
            $vars['product_price'] = $this->priceHelper->currency($product->getPrice(), true, false);
        }

        return $vars;
    }

    public function getProductImage($product, $imageId = 'category_page_grid')
    {
    ...
    public function getDemoSubscriber($store)
    {
